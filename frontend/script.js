...
playerForm.addEventListener('submit', (event) => {
    ...
    if (playerName === '' || playerName.length > 30) {
        nameError.textContent = "Por favor ingresa un nombre válido (máximo 30 caracteres)";
...
async function siguientePregunta() {
    ...
    try {
        ...
        gameContainer.innerHTML = `
            ...
            ${preguntaActual.bandera ? `<img src="${preguntaActual.bandera}" width="100" />` : ''}
            <div class="options">
        ...
        gameContainer.innerHTML = '<p>Error al cargar la pregunta. Inténtalo de nuevo.</p>';
    }
}
...
function terminarJuego() {
    ...
    resultadoUsuario.tiempoTotal = ((finTiempo - resultadoUsuario.inicioTiempo) / 1000).toFixed(2);
...
async function mostrarRanking() {
    try {
        ...
        if (ranking.length === 0) {
            rankingList.innerHTML = "<p>No hay jugadores registrados aún.</p>";
        } else {
